...
def make_Layers(depth, n_channels=64, out_channels=2, kernel_size=3, padding=1):
    ...
    for x in range(depth - 2):
        layers += [nn.Conv2d(in_channels=n_channels, out_channels=n_channels, kernel_size=kernel_size, padding=padding, stride = 1, bias=True),nn.BatchNorm2d(n_channels, eps=0.0001, momentum=0.9), nn.ReLU(inplace=True)]
    layers += [nn.Conv2d(in_channels=n_channels, out_channels=out_channels, kernel_size=kernel_size, padding=padding, stride = 1, bias=True)]
